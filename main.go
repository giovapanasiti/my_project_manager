					...
					if len(m.list.Items()) > 0 {
						...
						if ok {
							...
							if _, err := os.Stat(projectPath); err == nil {
								...
								m.fileChart = scanDirectory(projectPath, 3, 0, "")
								// Count file types and assign colors
								m.fileTypeCounts = countFileTypes(m.fileChart)
							}
						}
					}
					return m, nil
...
// Should a file/directory be excluded from the file chart?
...
// Scan the directory and build a file chart
...
// Get counts and colors for file types
...
// Render file chart and type statistics, simplified to only show the top 10 file types
...
func (m listModel) actionView() string {
	...
	if len(m.fileChart) > 0 && len(m.fileTypeCounts) > 0 {
		b.WriteString(renderFileChart(m.fileChart, m.fileTypeCounts))
		b.WriteString("\n")
	}
